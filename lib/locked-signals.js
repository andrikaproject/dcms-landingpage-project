...
// In-memory throttle: max 1 refresh per user per 20 seconds
const refreshThrottle = new Map();
const REFRESH_COOLDOWN_MS = 20_000;

function canRefresh(userEmail) {
    const last = refreshThrottle.get(userEmail) ?? 0;
    if (Date.now() - last < REFRESH_COOLDOWN_MS) return false;
    refreshThrottle.set(userEmail, Date.now());
    return true;
}
...
export async function refreshLockedSignalsForUser(userEmail) {
    ...
    // Skip exchange API calls if throttled — return cached DB state instead
    if (!canRefresh(userEmail)) {
        const cached = await db.query.lockedSignals.findMany({
            where: and(
        ...
        return cached.map(serializeLockedSignal);
    }
