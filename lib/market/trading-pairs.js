...
import { readCachedPairs, usableTradingPairs, writeCachedPairs } from "./trading-pairs-core";
...
export async function loadTradingPairs({ force = false, signal } = {}) {
    ...
    const items = usableTradingPairs(data);
    if (items.length === 0) throw new Error("Daftar coin aktif Bitunix kosong.");
