...
// Short-term memory cache for forceRefresh calls (10 seconds)
const memCache = new Map();
const MEM_CACHE_TTL = 10_000;

export async function bitunixFetch(path, forceRefresh = false, signal = undefined) {
    const url = `${BITUNIX_BASE_URL}${path}`;

    // Return memory-cached result if still fresh (avoids burst rate-limit on repeated forceRefresh)
    if (forceRefresh) {
        const cached = memCache.get(url);
        if (cached && Date.now() - cached.ts < MEM_CACHE_TTL) {
            console.log(`[BITUNIX] MemCache hit — ${url}`);
            return cached.data;
        }
    }
