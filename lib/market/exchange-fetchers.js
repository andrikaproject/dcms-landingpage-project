...
} from "@/lib/market/bitunix-request-gate";
import { createBitunixResponseCache } from "@/lib/market/bitunix-response-cache";
...
// In-flight deduplication: concurrent requests to the same URL share one pending fetch
const inFlight = new Map();
const requestGate = createBitunixRequestGate();
const responseCache = createBitunixResponseCache();

export async function bitunixFetch(path, forceRefresh = false, signal = undefined) {
    const url = `${BITUNIX_BASE_URL}${path}`;

    if (!forceRefresh) {
        const cached = responseCache.read(url);
        if (cached) {
            console.log(`[BITUNIX] Cache hit — ${url}`);
            return cached;
        }
    }
    ...
    if (inFlight.has(url)) {
        console.log(`[BITUNIX] Dedup in-flight — ${url}`);
    ...
    const promise = (async () => {
        try {
            ...
            const data = await withBitunixFrequencyRetries({
                signal,
                request: async () => requestGate.run(async () => {
                    console.log(`[BITUNIX] Fetching: ${url}`);
                    const response = await fetch(url, options);

                    if (!response.ok) {
                        console.error(`[BITUNIX] HTTP Error ${response.status} — ${url}`);
                        const error = new Error(`Bitunix responded ${response.status}`);
                    ...
                    if (body.code !== 0) {
                        console.error(`[BITUNIX] API Error code=${body.code} msg="${body.msg}" — ${url}`);
                        throw new Error(body.msg || "Bitunix API error");
                    }
                    return body;
                }, signal),
            });
...
// Instrument metadata for every Bitunix futures pair, including tokenized
// stocks/commodities (e.g. TSLAUSDT, XAUUSDT) that Bitunix lists alongside
// crypto perpetuals. Bitunix marks these with isApiSupported: false.
export async function fetchBitunixTradingPairs(forceRefresh = false) {
    const data = await bitunixFetch("/api/v1/futures/market/trading_pairs", forceRefresh);
    return Array.isArray(data?.data) ? data.data : [];
}

export async function fetchBitunixCandles(symbol, limit = 100, forceRefresh = false) {
    const data = await bitunixFetch(
        `/api/v1/futures/market/kline?symbol=${symbol}&interval=1d&limit=${limit}`,
        forceRefresh
    );
    const klines = data.data;
    if (!Array.isArray(klines) || klines.length === 0) return [];
    return normalizeBitunixKlines(klines, "1d");
}
