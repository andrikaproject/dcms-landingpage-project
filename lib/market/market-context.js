...
import { calculateUsdtDominanceTrendContext, getNeutralUsdtDominanceTrendContext } from "@/lib/market/usdt-dominance-trend";
...
function getHistory() {
    if (!globalThis.__usdtDomHistory) globalThis.__usdtDomHistory = [];
    return globalThis.__usdtDomHistory;
}
...
function findSnapshotNear(history, targetMs) {
    ...
    // reject if too far from target (>50% of window)
    return best && bestDiff <= targetMs * 0.5 ? best : null;
}

function computeChangePct(current, history, windowMs) {
    const past = findSnapshotNear(history, windowMs);
    return past ? current - past.value : null;
}

export async function getUsdtDominance(forceRefresh = false) {
    try {
        const result = formatUsdtDominance(await getCoinMarketCapUsdtDominance(forceRefresh), "coinmarketcap");
        pushSnapshot(result.value);
        return result;
    } catch {
        try {
            const result = formatUsdtDominance(await getCoinGeckoUsdtDominance(forceRefresh), "coingecko");
            pushSnapshot(result.value);
            return result;
        } catch {
...
export function getUsdtDominanceTrendContext() {
    const history = getHistory();
    if (history.length < 2) return getNeutralUsdtDominanceTrendContext("USDT.D trend data unavailable");

    const current = history[history.length - 1].value;
    const change4h = computeChangePct(current, history, WINDOW_4H_MS);
    const change1h = computeChangePct(current, history, WINDOW_1H_MS);
    const change15m = computeChangePct(current, history, WINDOW_15M_MS);

    return calculateUsdtDominanceTrendContext({
        "4h": change4h !== null ? { changePct: change4h } : {},
        "1h": change1h !== null ? { changePct: change1h } : {},
        "15m": change15m !== null ? { changePct: change15m } : {},
    });
}
