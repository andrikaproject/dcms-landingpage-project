...
const TOP_CMC_COINS = ["BTCUSDT", "ETHUSDT", "SOLUSDT", "BNBUSDT", "XRPUSDT", "DOGEUSDT", "ADAUSDT", "TRXUSDT"];
const MAJOR_COINS = [
    "BTCUSDT",
    "ETHUSDT",
    "SOLUSDT",
...
function formatUsdtDominance(usdtDominance = 0) {
    if (usdtDominance > 6.5) return { value: usdtDominance, label: "OVERBOUGHT", market: "MARKET BEARISH", score: -2 };
    if (usdtDominance < 4.5) return { value: usdtDominance, label: "OVERSOLD", market: "MARKET BULLISH", score: 2 };
    if (usdtDominance >= 5.5) return { value: usdtDominance, label: "RISING", market: "MARKET CAUTION", score: -1 };
    if (usdtDominance <= 5.0) return { value: usdtDominance, label: "FALLING", market: "MARKET POSITIVE", score: 1 };
    return { value: usdtDominance, label: "NEUTRAL", market: "MARKET NEUTRAL", score: 0 };
}
...
async function getUsdtDominance(forceRefresh = false) {
    try {
        ...
        return formatUsdtDominance(json.data.market_cap_percentage.usdt);
    } catch {
        return formatUsdtDominance(0);
    }
}
...
function processSignalData({ symbol, timeframe, usdtDomScore, closes, highs, lows, volumes, change, volume24h, source }) {
    ...
    if (stoch.k > stoch.d && stoch.k < 80) score += 1;
    else if (stoch.k < stoch.d) score -= 1;
...
export async function getMarketDashboard({ timeframe = "15m", limit = 12, symbol, reanalyze } = {}) {
    const safeTimeframe = intervalMap[timeframe] || "15m";
    const forceRefresh = Boolean(reanalyze);
    const searchedSymbol = normalizeSymbol(symbol);
    const [usdtDominance, extraTokens, btcTicker, ethTicker] = await Promise.all([
        getUsdtDominance(forceRefresh),
        getTopTokensByVolume(4, forceRefresh),
        binanceFetch("/api/v3/ticker/price?symbol=BTCUSDT", forceRefresh).catch(() => null),
        binanceFetch("/api/v3/ticker/price?symbol=ETHUSDT", forceRefresh).catch(() => null),
    ]);
    ...
    const settledSignals = await Promise.allSettled(
        symbols.map((symbol) => getSignal(symbol, safeTimeframe, usdtDominance.score, forceRefresh))
    );
