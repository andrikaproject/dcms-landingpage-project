...
export async function getUserSignalHistory({
    ...
    const rows = await db
        .select({
            ...
            marketType: signalSnapshots.marketType,
            bias: signalSnapshots.bias,
            entry: signalSnapshots.entry,
            currentPriceAtSignal: signalSnapshots.currentPriceAtSignal,
            sl: signalSnapshots.sl,
            tp1: signalSnapshots.tp1,
            tp2: signalSnapshots.tp2,
            rsi: signalSnapshots.rsi,
            emaFast: signalSnapshots.emaFast,
            emaSlow: signalSnapshots.emaSlow,
            fastPeriod: signalSnapshots.fastPeriod,
            slowPeriod: signalSnapshots.slowPeriod,
            stochK: signalSnapshots.stochK,
            stochD: signalSnapshots.stochD,
            riskPercent: signalSnapshots.riskPercent,
            rewardPercent: signalSnapshots.rewardPercent,
    ...
    return {
        items: items.map((row) => ({
            ...
            outcomeResolvedAt: row.outcomeResolvedAt instanceof Date ? row.outcomeResolvedAt.toISOString() : (row.outcomeResolvedAt ?? null),
